...
predicted_runs <- predict(m1, newdata = data.frame(at_bats = 5579))

observed_runs <- mlb11 %>%
  filter(at_bats == 5579) %>%
  select(runs)

residual <- observed_runs - predicted_runs
residual
```
...
multiple_model <- lm(runs ~ at_bats + hits + homeruns + strikeouts + stolen_bases + wins + bat_avg, data = mlb11)
...
multiple_model_new <- lm(runs ~ new_obs + new_slug + new_onbase, data = mlb11)
summary(multiple_model_new)
