...
nc %>%
  group_by(mature) %>%
  summarise(mean_gained = mean(gained, na.rm = TRUE), count = n())
...
summary_stats <- nc %>%
  group_by(mature) %>%
  summarise(min_age = min(mage, na.rm = TRUE),
...
max_younger_age <- summary_stats %>% filter(mature == "younger mom") %>% pull(max_age)
min_mature_age <- summary_stats %>% filter(mature == "mature mom") %>% pull(min_age)
