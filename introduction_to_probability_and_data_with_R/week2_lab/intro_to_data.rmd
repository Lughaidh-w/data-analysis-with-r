...
avg_speed <- nycflights %>%
  mutate(avg_speed = distance / (air_time / 60)) %>% # Calculate avg_speed in mph
...
nycflights <- nycflights %>%
  mutate(avg_speed = distance / (air_time / 60)) # Convert air_time from minutes to hours
...
on_time_arrival_percentage <- nycflights %>%
  group_by(dep_type) %>%
  summarise(on_time_arrival = mean(arr_type == "on time"))

print(on_time_arrival_percentage)

fraction_delayed_dep_arrive_on_time <- nycflights %>%
  filter(dep_type == "delayed") %>%
  summarise(fraction_on_time = mean(arr_type == "on time"))

print(fraction_delayed_dep_arrive_on_time)
```